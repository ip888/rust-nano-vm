...
share previews. Each marketing route (`/`, `/pricing`,
`/why-nanovm`, `/marketplace`) ships a programmatically-generated
1200×630 OG image (`app/**/opengraph-image.tsx`); Next.js resolves
relative image URLs against `metadataBase`, which we build from
this env var. Defaults to `https://nanovm.example.com` so a fresh
clone renders correctly; override with your own origin so
Twitter / LinkedIn / HN unfurls point at the right host. Blog posts
under `docs/blog/` are Markdown, not Next.js routes — they don't get
per-post OG images from this pipeline; GitHub renders them with its
own repo-level preview card.
