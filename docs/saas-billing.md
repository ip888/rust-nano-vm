...
- **Dunning**: `invoice.payment_failed` is logged but doesn't yet gate the customer's rate limit. Stripe's own `customer.subscription.updated` with `status: past_due` does propagate through the normal enforcement path.
